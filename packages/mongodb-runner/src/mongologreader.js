...
export async function* createLogEntryIterator(stdout) {
 ...
 * @returns The parsed line information.
 */
export function parseAnyLogEntry(line) {
  try {
...
export function isFailureToSetupListener(entry) {
  return (
...
function getPortFromLogEntry(logEntry) {
  ...
  if (isFailureToSetupListener(logEntry)) {
    throw new Error(
      `Failed to setup listener (${logEntry.message} ${JSON.stringify(logEntry.attr)})`,
    );
  }
  return -1;
}
 ...
 */
export async function filterLogStreamForPort(input) {
  let port = -1;
 ...
 */
export async function filterLogStreamForBuildInfo(input) {
