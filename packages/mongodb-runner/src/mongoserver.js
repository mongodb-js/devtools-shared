...
function getKeyFileOption(args) {
...
export class MongoServer extends EventEmitter {
  get id() {
    return this.uuid;
  }
  constructor() {
    ...
    this.hasInsertedMetadataCollEntry = false;
    this.isArbiter = false;
    this.isMongos = false;
    this.isConfigSvr = false;
    /* see .start() */
  }
  serialize() {
    return {
    ...
    if (options.binary === 'mongos' && !options.args?.includes('--port')) {
      ...
      while (true) {
        try {
          ...
          if (err.errorLogEntries.some(isFailureToSetupListener)) {
  ...
  async _ensureMatchingMetadataColl(client, mode) {
  ...
  async _populateBuildInfo(mode, clientOpts) {
    try {
  ...
  async assertHasInsertedLocalMetadata() {
    if (!this.hasInsertedMetadataCollEntry) {
