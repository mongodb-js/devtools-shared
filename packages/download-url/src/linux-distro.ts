...
export async function getCurrentLinuxDistro(): Promise<
...
> {
  ...
  let osReleaseId: string | undefined;
  try {
  ...
  if (osRelease) {
    ...
    if (id && (version || codename)) {
      ...
      const results = listDistroIds({ id, version: version || '', codename });
      if (results.length > 0) {
        return results;
      }
    }
...
function listDistroIds({
  ...
  switch (id.toLowerCase()) {
    case 'ubuntu': {
      ...
      if (major > 24)
        results.push({
          value: 'ubuntu' + version.replace('.', '') + '04',
          priority: 700,
        });
      return results;
    }
    case 'debian': {
      ...
      if (!version) {
        if (codename === 'buster')
          results.push({ value: 'debian10', priority: 100 });
        if (codename === 'bullseye')
          results.push({ value: 'debian11', priority: 200 });
        if (codename === 'bookworm')
          results.push({ value: 'debian12', priority: 300 });
        if (codename === 'trixie')
          results.push({ value: 'debian13', priority: 400 });
        if (codename === 'forky')
          results.push({ value: 'debian14', priority: 500 });
      }
      return results;
    }
    case 'suse':
    ...
    case 'amazon':
      if (version.match(/^201[0-9]\./)) {
        return [
          { value: 'amazon', priority: 100 },
          { value: 'amzn64', priority: 100 },
        ];
      } else if (version.match(/^\d\d\d\d\./)) {
        return [
          { value: 'amazon' + version.replace(/\..+$/, ''), priority: 100 },
        ];
      } else {
...
async function lsbReleaseInfo(): Promise<{
  id: string;
  version: string;
  codename: string;
}> {
  const [id, version, codename] = await Promise.all([
    (async () => {
      return (
        await execFile('lsb_release', ['-si'], { encoding: 'utf8' })
      ).stdout.trim();
    })(),
    (async () => {
      return (
        await execFile('lsb_release', ['-sr'], { encoding: 'utf8' })
      ).stdout.trim();
    })(),
    (async () => {
      return (
        await execFile('lsb_release', ['-sc'], { encoding: 'utf8' })
      ).stdout.trim();
    })(),
  ]);
