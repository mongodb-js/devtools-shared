...
type SchemaAnalysisType =
  | SchemaAnalysisBaseType
  | SchemaAnalysisNullType
  | SchemaAnalysisPrimitiveType
...
function isArrayType(
  type: SchemaAnalysisType,
): type is SchemaAnalysisArrayType {
  return (type as SchemaAnalysisArrayType).name === 'Array';
}

function isDocumentType(
  type: SchemaAnalysisType,
): type is SchemaAnalysisDocumentType {
...
function schemaToPaths(
  ...
  for (const field of sortedFields) {
    ...
    // Recurse on doc.
    const doc = Object.values(field.types).find(
      (f) => f.bsonType === 'Document',
    ) as SchemaAnalysisDocumentType | undefined;
    ...
    // Recurse on array.
    const array = Object.values(field.types).find(
      (f) => f.bsonType === 'Array',
    ) as SchemaAnalysisArrayType | undefined;
    if (array) {
      const arrayDoc = Object.values(array.types).find(
        (f) => f.bsonType === 'Document',
      ) as SchemaAnalysisDocumentType | undefined;
...
function simplifiedSchema(fields: SchemaAnalysisFieldsMap): SimplifiedSchema {
  function finalizeSchemaFieldTypes(
    types: SchemaAnalysisFieldTypes,
  ): SimplifiedSchemaType[] {
    return Object.values(types)
      .sort((a: SchemaAnalysisType, b: SchemaAnalysisType) => {
      ...
      .map((type: SchemaAnalysisType) => {
        return {
          ...
          ...(isArrayType(type)
            ...
            : {}),
          ...(isDocumentType(type)
            ? { fields: finalizeDocumentFieldSchema(type.fields) }
            : {}),
        };
      });
  }

  function finalizeDocumentFieldSchema(
    fieldMap: SchemaAnalysisFieldsMap,
  ): SimplifiedSchema {
...
function computeHasDuplicatesForType(
  type: SchemaAnalysisType,
  unique?: number,
) {
  if (isNullType(type)) {
...
function finalizeSchema(schemaAnalysis: SchemaAnalysisRoot): SchemaField[] {
    ...
    return Object.values(types)
      .map((type) => {
        ...
        return {
          ...
          ...(isDocumentType(type)
            ? { fields: finalizeDocumentFieldSchema(type.fields, type.count) }
            : {}),
        };
      })
  ...
  function finalizeDocumentFieldSchema(
    ...
    return Object.values(fieldMap)
      .map((field: SchemaAnalysisField): SchemaField => {
        ...
        return {
          ...
          type:
            fieldTypes.length === 1
              ? fieldTypes[0].name
              : fieldTypes.map((v: SchemaType) => v.name), // Or one value or array.
  ...
  return finalizeDocumentFieldSchema(
    schemaAnalysis.fields,
    schemaAnalysis.count,
  );
}
...
export class SchemaAnalyzer {
  ...
  increaseFieldCount() {
    ...
    if (this.fieldsCount > this.options.distinctFieldsAbortThreshold) {
      throw new Error(
        `Schema analysis aborted: Fields count above ${this.options.distinctFieldsAbortThreshold}`,
      );
    }
  }
  ...
  async analyzeDoc(doc: Document) {
     ...
     */
    const addToType = async (
      path: string[],
      value: BSONValue,
      schema: SchemaAnalysisFieldTypes,
    ) => {
      ...
      const typeName = this.options.semanticTypes
        ? this.getSemanticType(value, path) || bsonType
        : bsonType;
      if (!schema[typeName]) {
        schema[typeName] = {
          name: typeName,
      ...
      if (isArrayType(type)) {
        ...
        type.values.pushSome(
          getCappedValue(
            type.bsonType,
            value,
            this.options.storedValuesLengthLimit,
          ),
        );
      }
    };
    ...
    const addToField = async (
      ...
      path: string[],
      value: BSONValue,
      schema: SchemaAnalysisFieldsMap,
    ) => {
      await this.allowAbortDuringAnalysis();
      if (!schema[fieldName]) {
        schema[fieldName] = {
...
export function verifyStreamSource(source: AnyIterable): AnyIterable {
