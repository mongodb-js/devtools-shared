...
const createConvertInternalToStandard = function () {
  ...
  function getUsedDefinitions() {
    const filteredDefinitions = Object.fromEntries(
      Object.entries(RELAXED_EJSON_DEFINITIONS).filter(([key]) =>
        usedDefinitions.has(key),
      ),
    );
  ...
  async function parseType(
    ...
    switch (type.bsonType) {
      case 'Array':
        schema.items = await parseTypes(
          (type as ArraySchemaType).types,
          signal,
        );
        break;
      case 'Document':
  ...
  function isPlainTypesOnly(
    types: StandardJSONSchema[],
  ): types is { type: JSONSchema4TypeName }[] {
    return types.every(
      (definition) => !!definition.type && Object.keys(definition).length === 1,
    );
  }
  ...
  async function parseTypes(
    ...
    await allowAbort(signal);
    const definedTypes = types.filter(
      (type) => type.bsonType.toLowerCase() !== 'undefined',
    );
    const isSingleType = definedTypes.length === 1;
    if (isSingleType) {
      return parseType(definedTypes[0], signal);
    }
    const parsedTypes = await Promise.all(
      definedTypes.map((type) => parseType(type, signal)),
    );
  ...
  return async function convert(
    ...
    const { required, properties } = await parseFields(
