...
describe('options', function () {
  ...
  context('when using default options', function () {
    ...
    it('stores values by default', function () {
      assert.equal(
        (schema.fields[0].types[0] as PrimitiveSchemaType).values.length,
        3,
      );
    });
    ...
    it('does not use semantic type detection', function () {
      assert.equal(
        schema.fields.find((v) => v.name === 'email')?.types[0].name,
        'String',
      );
      assert.equal(
        (schema.fields.find((v) => v.name === 'email')?.types[0] as any)
          .bsonType,
        'String',
      );
      assert.equal(
        schema.fields.find((v) => v.name === 'shape')?.types[0].name,
        'Document',
      );
      assert.equal(
        (schema.fields.find((v) => v.name === 'shape')?.types[0] as any)
          .bsonType,
        'Document',
      );
    });
  ...
  context('when `semanticTypes` is an object', function () {
    ...
    context('and all values are custom detector functions', function () {
      ...
      it('uses the custom type detectors', function () {
        assert.equal(
          schema.fields.find((v) => v.name === 'is_verified')?.types[0].name,
          'Verification',
        );
        assert.equal(
          (schema.fields.find((v) => v.name === 'is_verified')?.types[0] as any)
            .bsonType,
          'Boolean',
        );
        assert.equal(
    ...
    context(
      ...
      function () {
        ...
        it('uses the enabled and custom type detectors', function () {
          ...
          assert.equal(
            (
              schema.fields.find((v) => v.name === 'is_verified')
                ?.types[0] as any
            ).bsonType,
            'Boolean',
          );
