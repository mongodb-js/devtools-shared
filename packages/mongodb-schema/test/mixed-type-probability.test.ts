...
describe('mixed type probability', function () {
  ...
  it('should compensate for missed Undefined values', function () {
    ...
    assert.equal(
      assigned?.types.find((v) => v.name === 'Boolean')?.probability,
      0.25,
    );
  });
});
