...
describe('mixed types nested', function () {
  ...
  before(async function () {
    ...
    valid = (
      schema.fields
        ...
        ?.types.find((x) => x.name === 'Document') as DocumentSchemaType
    )?.fields.find((v) => v.name === 'valid');
    if (!valid) {
  ...
  it('should see the `address` field is always present', function () {
    assert.equal(
      schema.fields.find((v) => v.name === 'address')?.probability,
      1,
    );
  });
  ...
  it('should see `Number` was used in 20% of documents', function () {
    assert.equal(
      valid?.types.find((v) => v.name === 'Number')?.probability,
      0.2,
    );
  });
  it('should see `Boolean` was used in 40% of documents', function () {
    assert.equal(
      valid?.types.find((v) => v.name === 'Boolean')?.probability,
      0.4,
    );
  });
  it('should see `Undefined` was used in 20% of documents', function () {
    assert.equal(
      valid?.types.find((v) => v.name === 'Undefined')?.probability,
      0.2,
    );
  });
  it('should see `String` was used in 20% of documents', function () {
    assert.equal(
      valid?.types.find((v) => v.name === 'String')?.probability,
      0.2,
    );
  });
});
