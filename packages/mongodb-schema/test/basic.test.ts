...
describe('using only basic fields', function () {
  ...
  it('should detect the correct type for each field', function () {
    ...
    assert.equal(
      users.fields.find((v) => v.name === 'apple_push_token')?.type,
      'String',
    );
    assert.equal(
      users.fields.find((v) => v.name === 'created_at')?.type,
      'Date',
    );
    assert.equal(users.fields.find((v) => v.name === 'email')?.type, 'String');
    assert.equal(
      users.fields.find((v) => v.name === 'is_verified')?.type,
      'Boolean',
    );
    assert.equal(users.fields.find((v) => v.name === 'length')?.type, 'Number');
    assert.equal(
      users.fields.find((v) => v.name === 'last_address_latitude')?.type,
      'Null',
    );
    assert.equal(
      users.fields.find((v) => v.name === 'last_address_longitude')?.type,
      'Null',
    );
    assert.equal(users.fields.find((v) => v.name === 'name')?.type, 'String');
    assert.equal(
      users.fields.find((v) => v.name === 'stats_friends')?.type,
      'Number',
    );
    assert.equal(
      users.fields.find((v) => v.name === 'twitter_username')?.type,
      'String',
    );
    assert.equal(
      users.fields.find((v) => v.name === 'toString')?.type,
      'Number',
    );
  });
});
