...
describe('hasDuplicates', function () {
  ...
  it('should not have duplicates', function () {
    assert.equal(
      schema.fields.find((v) => v.name === 'num')?.hasDuplicates,
      false,
    );
  });
  ...
  it('should have 10000 number values for the `num` field', function () {
    ...
    assert.equal(
      (types?.find((v) => v.name === 'Number') as PrimitiveSchemaType)?.values
        .length,
      10000,
    );
  });
  ...
  it('should have 100 string values for the `str` field', function () {
    ...
    assert.equal(
      (types?.find((v) => v.name === 'String') as PrimitiveSchemaType)?.values
        .length,
      100,
    );
  });
...
describe('unique', function () {
  ...
  it('should not have duplicates for `_id`', function () {
    assert.equal(
      schema.fields.find((v) => v.name === '_id')?.hasDuplicates,
      false,
    );
  });
  ...
  it('should not have duplicate values for b', function () {
    assert.equal(
      schema.fields.find((v) => v.name === 'b')?.hasDuplicates,
      false,
    );
  });
  ...
  it('should have duplicates for `registered`', function () {
    assert.equal(
      schema.fields.find((v) => v.name === 'registered')?.hasDuplicates,
      true,
    );
  });
});
