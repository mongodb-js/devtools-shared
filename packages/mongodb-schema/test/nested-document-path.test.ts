...
describe('nested document path', function () {
  ...
  it('should assemble the path correctly fields with dots in their names', function () {
    ...
    const bar = (
      foo?.types.find((v) => v.name === 'Document') as DocumentSchemaType
    )?.fields.find((v) => v.name === 'bar');
    const baz = (
      bar?.types.find((v) => v.name === 'Document') as DocumentSchemaType
    )?.fields.find((v) => v.name === 'baz');
    assert.ok(foo);
    assert.ok(bar);
    assert.ok(baz);
    ...
    const barWithDot = (
      foo.types.find((v) => v.name === 'Document') as DocumentSchemaType
    )?.fields.find((v) => v.name === 'bar.with.dot');
    assert.ok(barWithDot);
    const bah = (
      barWithDot.types.find((v) => v.name === 'Document') as DocumentSchemaType
    )?.fields.find((v) => v.name === 'bah');
    assert.ok(bah);
