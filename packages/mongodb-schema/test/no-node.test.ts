...
import type * as mongodbSchema from '..';

function createMockModuleSystem() {
  // `setTimeout` is available in every JS environment (browsers included), but
  // is not part of the ECMAScript spec and so is absent from a bare vm context.
  const context = vm.createContext(
    Object.assign(Object.create(null), { setTimeout }),
  );
  class Module {
    exports = {};
  }
  ...
  function makeRequire(basename: string) {
    return function require(identifier: string): any {
      if (
        ...
        throw new Error(
          `mock require() does not support Node.js built-ins (${identifier})`,
        );
      }
...
describe('getSchema should work in plain JS environment without Node.js or browser dependencies', function () {
  ...
  it('Check if return value is a promise', async function () {
    ...
    const { parseSchema } = makeRequire(__filename)(
      '../dist/index.js',
    ) as typeof mongodbSchema;
