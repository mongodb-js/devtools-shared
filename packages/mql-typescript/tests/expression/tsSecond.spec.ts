 ...
 * @see {@link https://www.mongodb.com/docs/manual/reference/operator/aggregation/tsSecond/#obtain-the-number-of-seconds-from-a-timestamp-field}
 */
function test0() {
  type stockSales = {
    _id: number;
  ...
  const aggregation: schema.Pipeline<stockSales> = [
    {
      $project: {
        _id: 0,
        saleTimestamp: 1,
        saleSeconds: { $tsSecond: '$saleTimestamp' },
      },
    },
  ];
}
...
function test1() {
  ...
  const aggregation: schema.Pipeline<stockSales> = [
    { $addFields: { clusterTimeSeconds: { $tsSecond: '$clusterTime' } } },
  ];
}
